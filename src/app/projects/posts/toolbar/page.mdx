...
- Smartsheet's core views (Board, Table, Timeline, etc.) are some of the most widely used surfaces in the entire platform. They all have one thing in common: a complex `Toolbar` component that offers some of the most important user actions in the platform.
- Our existing `Toolbar` had some limitations: its responsive behavior, focus management, and flexibility were all falling short of meeting teams' needs.
- Toolbar Buttons and other subcomponents were not built upon our basic `Button` component / existing components, leading to API drift and maintainability problems
...
# 💪🏽 Phase 2: Build out the new ToolbarV2 stuff!
